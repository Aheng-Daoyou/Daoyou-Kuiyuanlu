...
## 11. 增补：境界突破环节专项遍历（从闻腥·初期 → 渡渊·圆满，三轮，2026-09-04 深夜）

按用户要求对「修为/境界突破」环节做专项遍历：从最低境界到最高段位逐阶推进，排查
卡死 / 失败 / 内部错误，以及突破前置任务是否卡死或与其它环节冲突。共三轮。

### 11.1 被测系统

- 突破入口 `POST /api/cultivator/retreat {action:'breakthrough'}`（SSE 响应，首个 `result`
  事件到达时 DB 已事务提交，可安全断连避开 LLM 故事流）。
- 境界序列：9 大境界（闻腥/守灯/窥渊/蚀体/忘川/执灯/掌灯/近神/渡渊） × 4 小阶
  （初期/中期/后期/圆满）= **36 阶**；`getNextStage` 负责小阶内递进与大境界跃迁。
- 大境界跃迁有**破境任务门**（`TaskService.getMajorBreakthroughGate`，仅 realm_stage='圆满'
  且下一大境界存在时生效），门状态实时重算，不满足则 409。
- 突破需修为 ≥ 当前 cap 的 60%；每次耗灯油 20；不耗寿元/年龄。

### 11.2 结果（三轮 HTTP 全真实链，零代码改动）

脚本 `.e2e-tmp/breakthrough-sweep.ts`（每轮 DB 快照重置 → 真实 HTTP 逐阶突破）：
**三轮共 38 条记录：PASS=36 / RETRY-FAIL=2（正常随机失败后重试成功）/ FAIL=0**。

| 验证点 | 三轮结论 |
|---|---|
| 小境界直通 | 闻腥 初期→中期→后期→圆满 每轮连续突破成功（chance 0.95） |
| 大境界破境任务门 | 闻腥·圆满→守灯 每轮被 409 `MAJOR_BREAKTHROUGH_TASK_REQUIRED` 干净拦截（非 500/非卡死） |
| 门拦截后无冲突 | 角色仍居闻腥·圆满（未越界损坏）；GET 角色资源 200；可正常闭关 cultivate —— **与其它环节无冲突** |
| 满足门后放行 | 注入「破境凝神」状态后，闻腥圆满→守灯初期突破成功（chance 0.86） |
| 多层前置 | 守灯圆满→窥渊：无状态 409；仅持破境凝神仍因缺功法/副本 409 —— 前置逐层正确、不 500 |
| 最高境界边界 | 渡渊·圆满 再突破 → 409「已达最高境界，无法继续突破」干净 |
| 修为不足边界 | <60% cap → 409「灯韵不足，无法突破」干净 |
| 突破失败路径 | R3 出现 roll>chance 失败（扣修为/窥悟），重试即成功 —— 失败不崩溃 |

### 11.3 引擎级 36 阶确定性遍历

脚本 `.e2e-tmp/breakthrough-engine-sweep.ts`（进程内调引擎 `attemptBreakthrough`，rng=0 强制
成功），从闻腥·初期逐阶推到渡渊·圆满共 35 次跃迁：**359 个断言全通过，0 中途异常**。逐阶
校验：突破必成功、境界正确推进、6 维属性成长（大阶+4/小阶+2）、未分配点数（+20/+10）、
大境界寿元加成（守灯+200…渡渊+8000）、exp_cap 结转、渡渊圆满再突破抛「已达最高境界」。

### 11.4 结论

- 境界突破从最低（闻腥·初期）到最高（渡渊·圆满）全程**无 500 / 无卡死 / 无内部错误 /
  无崩溃**，引擎推进数学完全正确。
- **破境任务门机制健壮**：作为前置任务仅在「大境界圆满→下一大境界」时正确拦截，返回规范
  409；拦截后角色状态完整、可继续游戏与其它环节，不存在"卡死或与其它环节冲突"问题；
  满足前置（如注入破境凝神）后门即放行，可正常突破。
- 突破失败路径也安全（扣修为/窥悟、不崩溃、可重试）。
- 测试产生的虚假履历（林默 17 条突破历史 + 4 条闭关记录）与境界/寿元改动已清理还原。
- 本轮零代码改动（纯验证），突破相关单测 21/21、scan:terms 通过。

---

## 12. 增补：破境任务系统专项遍历（闻腥→守灯 前置任务达成/解门，三轮，2026-09-05 凌晨）

接第 11 节（境界突破遍历确认门会正确拦截、不与突破冲突），进一步验证**突破前置任务本身**
能否顺畅达成并解门，会不会卡死/500/与突破环节冲突。三轮全真实 HTTP。

### 12.1 被测机制

- 破境任务 `wanjiedaoyou_cultivator_tasks`，category='breakthrough_major'；闻腥→守灯档任务
  「守灯前引」仅 1 objective：持有 active 的「破境凝神」(breakthrough_focus) 状态。
- **惰性建档**：记录只在角色处于「闻腥·圆满」后首次可写同步才 INSERT（闻腥初期不会建档）。
- GET `/api/tasks` 为只读快照；GET `/api/tasks/:id`、POST retreat、服香等为可写同步（落库）。
- 任务完成状态随实时上下文重算：`status_active` 是"实时回声"，状态过期任务会回退。

### 12.2 结果（三轮 24 条记录：PASS=24 / FAIL=0）

| 验证点 | 三轮结论 |
|---|---|
| 建档规则 | 闻腥·初期无破境任务（仅圆满建档，不提前困扰） |
| 门禁拦截 | 闻腥·圆满突破被 409 `MAJOR_BREAKTHROUGH_TASK_REQUIRED` 拦 |
| 任务建档 | 闻腥→守灯任务建档为 active |
| 任务达成 | 持「破境凝神」后 GET /api/tasks/:id 同步 → status=completed |
| 门禁解除 | 达成后突破放行（进入 SSE 突破流程，不再 gate 拦） |
| **防绕过/回退** | 状态移除后再同步 → 任务回退 active、门禁重关 409（杜绝"先拿状态后过期仍虚开"） |
| 完成可见 | 达成后任务出现在 GET /api/tasks?status=completed 列表 |

### 12.3 结论

- 突破前置任务（破境任务）**能正常建档、达成、解门**，全程无 500 / 无卡死 / 无异常。
- **防绕过机制健壮**：任务完成是"快照式、可回退"——破境凝神状态一旦过期/被消耗，任务自动
  回退 active、门禁重关，不会造成"带过期状态虚开突破"的越界漏洞。
