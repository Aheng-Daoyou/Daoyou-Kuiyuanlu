...
- 通用层不读取宗门数据，不解析路由，也不判断设施、任务、工作区或 NPC 类型。
- 宗门业务层通过 renderer registry 组合不同会话；复杂玩法仍在独立工作区中完成。
- 不建立服务端会话或持久聊天记录，权威状态继续由现有查询资源持有。

## 组件职责

- `RoomView` 只展示房间描述、人物、选中状态和详情区域。
- `NpcConversation` 只展示人物信息、消息、单句回应、忙碌和错误状态。
...
- 普通人物切换使用 history replace；无效角色键按未选中处理。
- 进入工作区时保留 `npc`，退出时移除 `workspace` 并恢复原人物。
- 炼丹、炼器和闭关恢复 `keeper`，神通工作区恢复 `instructor`。
- 矿场战斗恢复 `keeper`，宗门小比恢复 `marshal`，山门清扫恢复 `keeper`。
...
- 四个生产宗门必须显式覆盖已开放设施 NPC 的 ID、姓名和问候。
- 姓名用于塑造人物，身份栏用于解释工作；禁止用设施或职位关键词直接拼成姓名。
- 事务堂标准任务与设施操作术语保持统一，不因宗门主题改名。
- 问候应以一句自然话说明当前可办理的事，不解释姓名，不重复身份栏，也不输出内部 renderer、metric、设施或任务键。

## 扩展方式

新增房间时，先在 shared 标准目录定义角色语义与 renderer，再由各生产宗门提供人物姓名和问候，最后在对应页面 registry 注册业务会话。新增工作区或活动返回路径时，只扩展宗门业务路由适配器；不得在 `RoomView`、 `NpcConversation` 或 `useConversationSession` 中增加条件分支。
