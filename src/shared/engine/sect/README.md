...
交付任务每次都生成核心限制：丹药必有最低品质、丹类、功效和品相；法宝必有最低品质、部位和未装备要求；材料必有数量、最低品质和类型。完美词条与元素仍是可选的额外条件。丹药品相要求按下品 30%、中品 45%、上品 20%、完美 5% 生成，完美必须精确匹配，其余均表示最低品相。
...
- `weekly_bounty_battle`：独立周常“悬赏令·讨伐”，领取时随机锁定异宗、同大境界且成员关系有效的玩家完整战斗构筑；无候选时不创建任务。玩家经访宗舆图进入目标宗门专用山门，与目标 NPC 开战。
...
玩家目标快照保存来源角色、来源宗门、锁定时间和完整 `CultivatorCombatInput`；任务视图只返回名称、描述、宗门、境界与阶段摘要，不暴露技能和装备数据。候选只按 `realm` 匹配同大境界，阶段可以不同；玩家后续换装、突破、转宗、死亡或注销不会改变已领取任务中的目标。
